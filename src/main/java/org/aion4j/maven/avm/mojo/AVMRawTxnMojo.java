...
@Mojo(name = "raw-txn", requiresDependencyResolution = ResolutionScope.COMPILE_PLUS_RUNTIME)
public class AVMRawTxnMojo extends AVMLocalRuntimeBaseMojo {
    ...
    private void parseArgs() throws MojoExecutionException {
        ...
        keystore = ConfigUtil.getPropery("keystore");
        ...
        if(keystore != null)
            getLog().info("Sender Address   : " + keystore);
    ...
    private void printHelp() {
        getLog().info("Usage:");
        getLog().info("./mvnw  aion4j:raw-txn [-Dcontract=<contract_address>] [-Dkeystore=<keystore_path>] -Dpassword=[keystore_pwd] -Dmethod=<method_name> [-Dvalue=<value>] [-Dargs=<method_args>]");
    }
